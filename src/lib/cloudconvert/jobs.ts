import type { ConversionOptions, ImageFormat } from "@/types/conversion";
...
export function calculateProgress(job: CloudConvertJob): number {
  if (!job.tasks) return 0;

  const tasks = job.tasks;
  const totalTasks = tasks.length;
  const completedTasks = tasks.filter(
    (t: CloudConvertTask) => t.status === "finished" || t.status === "error",
  ).length;

  return Math.round((completedTasks / totalTasks) * 100);
}

export function getDownloadUrl(job: CloudConvertJob): string | undefined {
  const exportTask = job.tasks?.find(
    (t: CloudConvertTask) => t.name === "export-file",
  );
